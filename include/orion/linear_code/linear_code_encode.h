...
extern common::prime_field::field_element *scratch[2][100];

extern bool __encode_initialized;

inline int encode(const common::prime_field::field_element *src, common::prime_field::field_element *dst, long long n, int dep = 0)
{
    if(!__encode_initialized)
    {
        ...
        for(int i = 0; (n >> i) > 1; ++i)
        {
            scratch[0][i] = new common::prime_field::field_element[2 * n >> i];
            scratch[1][i] = new common::prime_field::field_element[2 * n >> i];
        }
    }
    ...
    for(long long j = 0; j < R; ++j)
        scratch[1][dep][j] = common::prime_field::field_element(0ULL);
    //expander mult
    for(long long i = 0; i < n; ++i)
    {
        const common::prime_field::field_element &val = src[i];
    ...
    {
        scratch[0][dep][n + L + i] = common::prime_field::field_element(0ULL);
    }
    for(long long i = 0; i < L; ++i)
    {
        common::prime_field::field_element &val = scratch[0][dep][n + i];
