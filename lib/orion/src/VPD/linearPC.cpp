...
::std::pair<prime_field::field_element, bool> tensor_product_protocol(prime_field::field_element *r0, prime_field::field_element *r1, long long size_r0, long long size_r1, long long N, __hhash_digest *com_mt)
{
    ...
    printf("[Orion] Query count %d\n", query_count);
    printf("[Orion] Column size %d\n", column_size);
    printf("[Orion] Number of merkle pathes %d\n", query_count);
    printf("[Orion] Number of field elements %d\n", query_count * column_size);
    ...
    printf("[Orion] Proof size for tensor IOP %lld bytes\n", proof_size + v.proof_size);
    printf("[Orion] Verification time %lf\n", verification_time);
