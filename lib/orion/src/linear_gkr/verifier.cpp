...
#include <vector>
#include <iostream>
#include "lib/orion/include/linear_gkr/random_generator.h"
#include "lib/orion/include/VPD/vpd_verifier.h"
...
void zk_verifier::read_circuit(const char *path, const char *meta_path)
{
	...
	for(int i = 1; i <= d; ++i)
	{
		...
		for(int j = 0; j < n; ++j)
		{
			...
			if(g != previous_g + 1)
			{
				printf("[Orion] Error, gates must be in sorted order, and full [0, 2^n - 1]. %d %d %d %d\n", i, j, g, previous_g);
				exit(0);
			}
			previous_g = g;
			if(i != 1)
...
bool zk_verifier::verify(const char* output_path)
{
	...
	printf("[Orion] GKR witness size %d\n", 1 << C.circuit[0].bit_length);
